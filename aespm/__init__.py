	...
	try:
		...
		if not os.path.exists(os.path.join(buffer_path, 'SendToIgor.bat')):
			with open(os.path.join(buffer_path, 'SendToIgor.bat'), 'w') as fopen:
				if os.path.exists("C:\\Asylum Research\\v19"):
					fopen.write('"C:\\Asylum Research\\v19\\RealTime\\Igor Pro Folder\\Igor.exe" "{}"'.format(os.path.join(buffer_path, 'ToIgor.arcmd')))
				elif os.path.exists("C:\\Asylum Research\\v18"):
					fopen.write('"C:\\Asylum Research\\v18\\RealTime\\Igor Pro Folder\\Igor.exe" "{}"'.format(os.path.join(buffer_path, 'ToIgor.arcmd')))
				elif os.path.exists("C:\\Asylum Research\\v17"):
					...
					fopen.write('"C:\\Asylum Research\\v13\\RealTime\\Igor Pro Folder\\Igor.exe" "{}"'.format(os.path.join(buffer_path, 'ToIgor.arcmd')))
				else:
